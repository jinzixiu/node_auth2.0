...
exports.ensureLogin = function (req, res, next) {
  ...
  var pathname = parseUrl(req.originalUrl,true).pathname;
  if(pathname==='/OAuth2/authorize'){
      var queryString = parseUrl(req.originalUrl,true).query;
      // console.log(queryString);
      var redirect_uri = queryString.redirect_uri;
      var client_id = queryString.client_id;
      var OAuth2_authorize_url = req.originalUrl;
  }

  res.locals.redirect_uri =redirect_uri;
  res.locals.client_id =client_id;
  res.locals.OAuth2_authorize_url =OAuth2_authorize_url;
  ...
  if(pathname==='/OAuth2/login'){
      console.log("#######################")
      console.log(req.body.OAuth2_authorize_url)
      console.log(req.loginUserId)
      // return res.redirect(req.body.OAuth2_authorize_url);
  }
  // req.loginUserId = 'doudou';
